...
class Block {
    constructor(x, y, type, color) {
        ...
        this.type = type // タイプ（normal or goal）
        this.color = color
...
const init = () => {
    ...
    for (let i=0; i<map.length; i++) {
        for (let j=0; j<map[i].length; j++) {
            ...
            if (char === "#") {
                blocks.push(new Block(j,i,"normal","orange"))
            } else if (char === "S") {
                sx = (j + 0.5) * blockSize
                sy = (i + 0.5) * blockSize;
                [x, y] = [sx, sy]
            } else if (char === "G") {
                blocks.push(new Block(j,i,"goal","gray"))
            }
        }
    }
...
const startGame = () => {
    ...
    startTime = Date.now()
    if (timer != null) crearInterval(timer)
...
const update = () => {
    ...
    let checkGoal = false
    const [r1, r2] = [ballRedius, blockSize / 2]
    ...
    if (checkGoal) {
        clearInterval(timer)
        timer = null
        playContext.font = "bold 100px sans-self"
...
const draw = () => {
    ...
    playContext.beginPath()
    playContext.arc(cx - r/3, cy - r/3, r/3, r/3, 0, Math.PI*2)
    playContext.fill()
}
